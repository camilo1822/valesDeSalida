		...
		<meta http-equiv="X-UA-Compatible" content="IE=Edge">
			...
			<link rel="stylesheet" type="text/css"
				...
				<link rel="stylesheet" type="text/css"
					...
					<body id="formVale" class="blockInspect valeDetail" ng-app="validationApp"
						...
						<input type="text" id="credencial" name="credencial"
							value="${model.credencial}" style="display: none" readonly /> <input
							type="text" id="credencialCorreo" name="credencialCorreo"
							value="${model.credencialCorreo}" style="display: none" readonly />
						...
						<div class="main-container">
							...
							<div class="input-block col-quarter noVale">
								<label class="label">NÚMERO DE VALE:</label> <input id="valeNum"
									...
									readonly /> <input id="nameJef" type="text" name="nameJef"
...
form input.ng-invalid.ng-touched {
	border-color: #FA787E;
							...
							<form id="salidaMaterialesForm" name="salidaMaterialesForm"
								method="post" action="valeEditar" novalidate>
								...
								<input type="text" id="descripcionUsuario"
									...
									type="text" id="correoFiltrado"
									value="${model.operacion.getUsuario().getEmail()}"
									Style="display: none" />
								<section>
												...
												<c:forEach items="${model.detVales}" var="i">
													<tr>
														...
														<td class="column-two"><input class="in-descri"
															type="text" value="<c:out value="${i.getCentro()}"/>"
															readonly /></td>
														<td class="column-two"><input class="in-descri"
															type="text" value="<c:out value="${i.getCodigo()}"/>"
															readonly /></td>
														<td class="column-three"><input
															id="descriList<c:out value="${i.getFila()}"/>"
															class="in-descri toolList" type="text"
														...
														<td ng-if="data.prorroga=='Si'"><input type="button"
															...
															class="botComent"
															ng-model="botCom<c:out value="${i.getFila()}"/>"
															onclick="coment('${i.getFila()}')" /> <textarea
																id="textCom<c:out value="${i.getFila()}"/>" rows=""
																cols="" Style="display: none">${i.getComentario()}</textarea>
														</td>

														<td ng-if="data.prorroga=='Si'" class="column-eleven">
															<input
															ng-if="'<c:out value="${i.getFechaProrroga()}"/>' == ''"
															id="fecProrroga<c:out value="${i.getFila()}"/>"
															class="fec datepicker" type="text"
															name="fecProrroga<c:out value="${i.getFila()}"/>"
															value="<c:out value="${i.getFechaProrroga()}"/>" readonly />
															<input
															ng-if="'<c:out value="${i.getFechaProrroga()}"/>' != ''"
															id="fecProrroga<c:out value="${i.getFila()}"/>"
															class="fec in-descri" type="text"
															name="fecProrroga<c:out value="${i.getFila()}"/>"
															value="<c:out value="${i.getFechaProrroga()}"/>" readonly />
														</td>
														...
														<td ng-if="data.prorroga=='Si'"><input
															...
															class="checkbox"
															ng-model="chekRecibidoPort<c:out value="${i.getFila()}"/>"
															ng-init="chekRecibidoPort<c:out value="${i.getFila()}"/>=false"
															ng-change="mostrarPort('${i.getFila()}','{{chekRecibidoPort<c:out value="${i.getFila()}"/>}}')" />
															<a ng-if="'<c:out value="${i.getPort()}"/>' != ''"
															class="recibido"></a> <label
															ng-if="'<c:out value="${i.getPort()}"/>' == ''"
															for="checkRecibidoPort<c:out value="${i.getFila()}"/>"
															class="elemento imgCheck imgCheckCond"
															id="checkRecibidoPort<c:out value="${i.getFila()}"/>" />
														</td>
														...
														<td class="column-six" style="display: none"><input
															id="fecFinalPort<c:out value="${i.getFila()}"/>"
															class="in-descri" type="text"
															name="fecFinalPort<c:out value="${i.getFila()}"/>"
															value="fechaFinalPort<c:out value="${i.getFila()}"/>"
															ng-model="fechaFinalPort<c:out value="${i.getFila()}"/>"
															ng-init="fechaFinalPort<c:out value="${i.getFila()}"/>='<c:out value="${i.getPort()}"/>'"
															readonly /> <input Style="Display: none"
															class="in-descri" type="text"
															name="fecFinali<Port<c:out value="${i.getFila()}"/>"
															value="fechaFinaliPort<c:out value="${i.getFila()}"/>"
														...
														<td id="campoAprobado"><input
															...
															type="checkbox"
															id="checkRecibido1<c:out value="${i.getFila()}"/>"
															class="enableAprobado"
															ng-model="chekRecibido1<c:out value="${i.getFila()}"/>"
															ng-change="recibido('{{chekRecibido1<c:out value="${i.getFila()}"/>}}','${i.getFila()}')" />
															<a ng-if="'<c:out value="${i.getAprobado()}"/>' == 'Si'"
															class="recibido"></a> <label
															for="checkRecibido1<c:out value="${i.getFila()}"/>"
															class="elemento imgCheck imgCheckCond"></label> <input
															type="text"
														...
														<td
															...
															title="Editar" onclick="CallPicker()"
															ng-click="editarMaterial()"></a> <a
															id="sinPort<c:out value="${i.getFila()}"/>"
															class="save ocultarr" title="Guardar"
															onclick="guardarMaterial(${i.getFila()})"
															ng-click="mensajeExito()"></a> <a
															id="conPort<c:out value="${i.getFila()}"/>"
															class="save ocultarr" title="Guardar"
															onclick="guardarMaterial2(${i.getFila()})"
								...
								<div class="form-row">
									...
									<div class="input-block col-quarter">
										<label class="label">Almac&eacute;n:</label> <input
											type="text" name="almacen" value="${model.vale.getAlmacen()}"
											readonly />
									</div>
									<div class="input-block col-quarter">
										<label class="label">Control porter&iacute;a:</label> <input
											type="text" name="controlPorteria"
											value="${model.vale.getPorteria()}" readonly />
									</div>
									<div class="input-block col-quarter">
				...
				<input type="submit" name="Guardar" value="Aprobar" class="submit" ng-click="submitForm()"/>
							...
							<form id="generarPdf" name="generarPdf" method="post"
								action="verPdf">
								<section class="buttons"> <!-- <input type="button" name="verPdf" value="Ver Pdf" class="pdf visualizar" onclick="pdf()"> -->
								...
								<div class="modal fade" id="modalRechazo" role="dialog">
									<div class="modal-dialog">
										...
										<div class="modal-content ocultar">
											<div>
												<h3 id="titJustifi">Comentario</h3>
											</div>
											...
											<div>
												<input type="button" id="btnRecha" name="aceptarRechazo"
													value="Aceptar" class="submit2 rechazado btnCom"
													onclick="envComentario()" /> <input type="button"
													id="btnRecha" name="aceptarRechazo" value="Cancelar"
													class="submit2 rechazado btnCom"
													onclick="cerrarModalRechazado()" />
											</div>
										</div>
									</div>
								</div>
